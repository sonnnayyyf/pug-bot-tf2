...
- **Off-node backups** — after every `/match report` (and on demand via `/backup`), the bot uploads a consistent snapshot of `pug.db` to a private backup channel, so Elo/records survive the host dying. Restore by downloading the newest `pug-*.db` and dropping it in as `pug.db`.
...
- **Player stats, Elo & records** — every completed game is counted per player, and every game *reported with a result* updates a per-player Elo rating plus a win/loss/draw record. Each player's rating change is computed against the opposing team's average (so a favorite gains less than an underdog), renormalized so the match is zero-sum, and capped at ±20 per game. A draw scores half a win each — balanced teams don't move, an uneven game nudges the favorite down and the underdog up. Everyone starts at 1600. `/captstat` shows the top 10 most-rolled captains, `/elo top` shows the top 10 by rating, and `/stat [@player]` shows someone's Elo, W–L–D, win rate and captaincies (win rate is `W/(W+L)`, so draws don't count against it). All counts are lifetime and shared across both lobbies. Elo is cosmetic — it never gates queuing or captain rolls, and admins can correct it with `/elo set` / `/elo add`.
- **Admin controls** — `/match report` (record the live game's winner), `/match cancel` (end a match with no result — void a live game or scrap a forming one), `/match put` (move a player to a team, a captain slot, or the bench — assigning a captain bumps the old one to unpicked; filling both teams this way completes the draft and goes live), `/match start` (force a fully-set draft to go live), `/match fix` (correct a misreported match — auto-recomputes Elo + W/L/D), `/immunity` (manage med immunity), `/elo set|add` (correct a rating), `/reset` (re-roll a stuck draft), `/clear` (clears the next queue mid-game, the active queue otherwise), and `/forceadd` (rebuild a queue, e.g. after a restart).
...
| `/backup` | Admin: snapshot `pug.db` and upload it to the backup channel now |
...
| `/stat [@player]` | A player's Elo, W–L–D, win rate and captaincies (defaults to you) |
...
| Command | What it does |
|---|---|
| `/match report red\|blu\|draw` | Record the result of the live game (captain/admin) — updates W/L/D + Elo. A result is required |
...
| `/match start` | Admin: force a fully-set draft to go live (e.g. after arranging teams with `/match put`) |
...
| `PICK_ORDER` | `1-1-1-2-1-1-1-1-1` | Per-turn pick counts (RED first; BLU's double is mid-draft) |
| `BACKUP_CHANNEL_ID` | (set) | Private channel the bot uploads `pug.db` backups to |
...
`pug_state.py` ships with a smoke-test suite (cases A–EE) covering the full rule set — pick-order balance, the auto-ready / ready-check paths (including arming auto-ready mid-check), medic immunity (no-stacking and exclusion from the roll), subs, captain step-down, the abort flow, the next queue (hold/promote/merge), `/match put` (including completing the draft and going live, and `/match start`), admin cancel, scoped `/clear`, last-player auto-assign, the no-auto-end rule (a live game stays live until reported), stats/Elo accumulation (recorded W/L/D, per-player ratings, renormalized zero-sum, the ±20 clamp, draws (balanced no-move + unbalanced convergence), admin void, match correction (re-grading a misreport from the audit log), legacy-record upgrade, the audit detail record), and a full JSON state round-trip for persistence. `lobby.py` and `storage.py` carry their own smaller suites (run `python lobby.py` / `python storage.py`):
